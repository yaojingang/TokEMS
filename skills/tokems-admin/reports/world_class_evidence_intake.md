...
| `human-adjudication` | `fix-submission` | `fail` | Collect three controlled reviewer packets and adjudicate them against the private run answer key. |
| `native-permission-enforcement` | `fix-submission` | `fail` | Integrate a real target-client or external installer runtime guard before claiming native permission enforcement. |
| `native-client-telemetry` | `fix-submission` | `fail` | Install a real client against the native host and import production metadata-only events. |
