...
- 异常：正式发布前发现 TokEMS 与同机 GEOFlow 容器停止；内核日志记录 `dockerd` 两次被 OOM 终止。现场只读诊断复现了 `docker system df` 触发守护进程常驻内存快速增长。发布完成后，`dockerd` 于 11:58 再次 OOM，公网在 12:05 收尾巡检时返回 502。
- 影响：第一次故障发生在正式发布开始前，数据库迁移、镜像标签和生产环境文件均未变更。第二次故障发生在发布后，TokEMS 容器随 Docker 守护进程停止，已发布镜像、数据库和恢复标记保持不变。
- 根因：宝塔 `BT-Panel` 的 Docker 总览接口调用 `docker system df --format json`。现场发现两条父进程为 `BT-Panel`、持续二十分钟以上的并发扫描进程；它们将旧 dockerd 推入 OOM，并在守护进程自动重启后继续造成快速内存增长。Docker 将退出容器记录为人工停止状态，因此守护进程重启后没有自动拉起应用。
- 处理：两次均终止准确的扫描进程、重启 Docker 守护进程并按依赖顺序恢复现有 TokEMS 容器。第二次处置进一步修补 `/www/server/panel/class/btdockerModel/statusModel.py`，改用 Docker info 的有界数量字段，原文件备份为 `statusModel.py.pre-tokems-20260903-1210`，随后只重启宝塔面板进程。未执行 prune、删卷、数据库覆盖恢复、迁移降级或镜像切换。GEOFlow 保持停止，留给其独立维护流程处理。
- 是否触发回滚：否；两次故障都没有改变已选镜像、数据库或环境文件，正式发布一次完成。

第二次恢复后，11 个 TokEMS 容器全部 healthy，本机与公网首页和健康接口均为 HTTP 200，运行提交仍为 `4579248e5eb645df530b999a9c955b6369ba58ca`，恢复标记不存在。`dockerd` RSS 从约 4.3 GiB 回落到约 125 MiB，持续观察期间没有重新出现扫描进程。

`docker system df` 及其 `-v` 变体已写入生产运维禁用项。磁盘检查改用 Docker Root Dir 对应文件系统的 `df`，镜像检查使用有界的 `docker image inspect`。宝塔升级可能覆盖现场修补，升级验收必须检查该调用没有恢复。
...
- 剩余风险：宝塔升级可能覆盖 Docker 总览现场修补；Docker 26.1.3 的升级或元数据治理仍需独立维护窗口；自动批准部署需等待另外两次不同 SHA 的预构建生产发布成功。
- 下次发布前事项：继续使用私有 GHCR 预构建路径，确认宝塔源码未恢复 `docker system df` 调用、没有遗留扫描进程、dockerd 内存稳定、恢复标记不存在，并完成第 2 次预构建发布计数。
