...
回滚还要求目标版本的整组原容器及其镜像身份仍然存在。槽位被后续发布复用后，较早版本的容器可能已被替换；此类历史目标会在暂停当前后台、写入恢复标记之前被拒绝。保留的镜像和发布记录用于人工恢复取证，当前入口不会自动重建已被替换的历史容器。

准备阶段被异常信号中断后，控制器可在同一个发布目录重新解包同一已验证目标的源码。目标控制器实现也必须与本次启动的实现一致；若目标修改了控制器，准备阶段会在更新服务器 checkout 后停止，旧服务继续运行。确认输出后，从已更新的 `/www/wwwroot/TokEMS` 重新执行同一目标的 `deploy`，新调用使用目标控制器。测试文件变动不触发这个门禁。不要对这类 `preparation-failed` 记录执行 `resume`。
